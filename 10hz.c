...
void setup()
{
    ...
    timer = timerBegin(0, 80, true); // ใช้ Timer 0, ตั้งค่าหารสัญญาณเป็น 80, และให้นับขึ้น (count up)

    /* เชื่อมฟังก์ชัน onTimer เข้ากับ Timer */
    timerAttachInterrupt(timer, &onTimer, true);

    /* ตั้งค่าให้ Timer เรียก onTimer ทุก ๆ 50,000 ไมโครวินาที (0.05 วินาที) */
    /* ค่า 1 tick = 1 ไมโครวินาที (us) => 50000 ticks = 50000 us = 0.05 วินาที */
    timerAlarmWrite(timer, 50000, true); // ตั้งค่า Timer และให้ทำงานซ้ำ (true)
